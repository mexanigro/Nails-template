      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            className="fixed bottom-24 right-6 z-50 w-14 h-14 bg-amber-600 hover:bg-amber-500 text-white rounded-full flex items-center justify-center shadow-[0_0_20px_rgba(217,119,6,0.3)] transition-all active:scale-95 group"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-[100] w-[380px] max-w-[calc(100vw-3rem)] h-[600px] max-h-[calc(100vh-7.5rem)] bg-zinc-50 dark:bg-zinc-950 transition-colors duration-300 border border-zinc-200 dark:border-zinc-800 transition-colors duration-300 rounded-3xl shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin scrollbar-thumb-zinc-800">
              {messages.map((msg) => (
                ...
                >
                  <div className={cn(
                    ...
                    msg.role === "user" ? "bg-amber-600 text-white" : "bg-zinc-100 dark:bg-zinc-800 transition-colors duration-300 text-amber-500"
                  )}>
                  ...
                  <div className={cn(
                    ...
                    msg.role === "user" 
                      ...
                      : "bg-white dark:bg-zinc-900 transition-colors duration-300 text-zinc-600 dark:text-zinc-300 transition-colors duration-300 border border-zinc-200 dark:border-zinc-800 transition-colors duration-300 rounded-tl-sm [&_p]:mb-2 [&_p:last-child]:mb-0 [&_ul]:list-disc [&_ul]:ml-4 [&_ul]:mb-2 [&_strong]:text-amber-400"
                  )}>
                    {msg.role === "model" ? (
                      <div>
              ...
              {isLoading && (
                <div className="flex gap-4 max-w-[85%] mr-auto">
                  <div className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 transition-colors duration-300 text-amber-500 flex items-center justify-center shrink-0 mt-1">
                    <Bot size={14} />
                  </div>
            ...
            <form onSubmit={handleSubmit} className="p-4 bg-white dark:bg-zinc-900 transition-colors duration-300 border-t border-zinc-200 dark:border-zinc-800 transition-colors duration-300">
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full bg-zinc-50 dark:bg-zinc-950 transition-colors duration-300 border border-zinc-200 dark:border-zinc-800 transition-colors duration-300 rounded-full pl-5 pr-14 py-3.5 text-sm text-zinc-950 dark:text-white placeholder-zinc-500 focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/50 transition-all font-medium"
                />
                <button
                  type="submit"
                  disabled={!input.trim() || isLoading}
                  className="absolute right-2 w-10 h-10 bg-amber-600 hover:bg-amber-500 text-white rounded-full flex items-center justify-center disabled:opacity-50 disabled:bg-zinc-100 dark:disabled:bg-zinc-800 disabled:text-zinc-500 transition-all"
